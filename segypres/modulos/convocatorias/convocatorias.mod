...
function listarConvs( $estado=null ) 
{
	...
	if (tienePermiso(ID_ROL_CONV, 'ver')) {
		$accs_cab .= " | " . enlace('?r=' . CONV_LISTAR_HTML, 'HTML', 
				"Mostrar el listado de convocatorias en formato HTML", "", "_BLANK") . "</h2>";
	}
	...
	if ($BD->numRows() > 0) {
		...
		while ($reg = $BD->getNext()) 
			...
			$acciones = array();
			if (tienePermiso(ID_ROL_CONV, 'ver')) {
				$acciones[] = enlace('?r=' . CONV_MOSTRAR . $id, 'Ver', "Ver los datos de la convocatoria: {$reg['objeto']} | {$reg['entidad']}");
			}
			if (tienePermiso(ID_ROL_MOD, 'ver')) {
				$acciones[] = enlace('?r=' . MOD_IMP_PRESUP . $id, 'Presupuesto', 
				"Imprimir el presupuesto de la convocatoria: {$reg['objeto']} | {$reg['entidad']}");
			}
                        
			if (tienePermiso(ID_ROL_MOD, 'ver')) {
				$acciones[] = enlace('?r=' . MOD_IMP_PRESUP_HTML . $id, 'HTML', 
				"Imprimir el presupuesto de la convocatoria: {$reg['objeto']} | {$reg['entidad']}", "", "_BLANK");
			}
			...
			. "<td class=\"{$reg['estado']}\" >{$reg['cuce']}</td>"
			. "<td class=\"{$reg['estado']}\" >{$reg['fecha_inicio']}</td>"
			. "<td class=\"{$reg['estado']}\" >{$reg['duracion']}</td>"
			. "<td class=\"{$reg['estado']}\" >{$reg['fecha_fin']}</td>"
			. "<td class=\"{$reg['estado']}\" >" . implode('|', $acciones)
			. "</td>"
			. "</tr>\n";
		}
		$html .= "</table>";
